...
def AUTH():
        ...
        if 'success' in html:
            ...
            if dialog.yesno("Savvy TV", winput[0],'', 'Please Try Again'):
                SIGNIN()
            else:
                EXIT()
...
def Launch():        
    ...
    if dialog.yesno("Savvy TV", 'Please call +34 633 079 305 To Register','', "Or Sign In",'Register','Sign In'):
        ...
        link=OPEN_URL(url)
        if 'success' in link:
            dialog.ok("Savvy TV", 'Thank You For Registering','Please Open Your Email Client', 'And Verify Your Email Address')
            ADDON.setSetting('user',email)
            ADDON.setSetting('pass',password)
            ADDON.setSetting('firstrun','true')
        else:
            winput=re.compile('"message":"(.+?)"').findall(link)
            if dialog.yesno("Savvy TV", winput[0],'', 'Exit And Restart Again'):
                Launch()
            else:
                EXIT()
