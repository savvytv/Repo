...
def CARDPAY(cat):
    dialog = xbmcgui.Dialog()
    if dialog.yesno("Savvy TV", "Are You Happy To Continue",'', "Please Input Card Details"):
        card_payment('thingy.xml',ADDON.getAddonInfo('path'),'DefaultSkin',cat=cat).show()
...
def PAYSUBS(cat):
    ...
    if 'success' in link:
        ...
        if dialog.yesno("Savvy TV", "You Have a Similar Subscription",'', "What Do You Want To Do","Extend","Create New"):
                print '######################################   CREATE NEW'
