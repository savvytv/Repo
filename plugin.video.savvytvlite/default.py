...
fanart = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.savvytvlite', 'fanart.jpg'))
iconart = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.savvytvlite', 'icon.png'))
channel_list = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.savvytvlite/helpers', 'channel.list'))
cartoonlinks = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.savvytvlite/helpers', 'cartoonlinks.list'))
group_list = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.savvytvlite/helpers', 'groups.list'))
xml_list = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.savvytvlite/helpers', 'FilmOn.xml'))
ct_list = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.savvytvlite/helpers', 'cartoons.list'))
...
def keep_session():
    currentWindow = xbmcgui.getCurrentWindowId()
        ...
        #print 'Savvy TV Lite..........logged out of Filmon'
        #return
    session_id = xbmcgui.Window(10000).getProperty("session_id")
    url = "http://www.filmon.com/api/keep-alive?session_key=%s" % (session_id)
    open_url(url)
    print 'Savvy TV Lite..........Filmon session kept alive'
...
if not xbmcgui.Window(10000).getProperty("session_id"):
    ...
    if FILMON_ACCOUNT:
        ...
        print "Savvy TV Lite......Logged in"
        xbmcgui.Window(10000).setProperty("session_id", session_id)
        keep_session()
    else:
        print "Savvy TV Lite......Not logged in"
        xbmcgui.Window(10000).setProperty("session_id", session_id)
        keep_session()
...
def CATEGORIES():
    ...
    addDir('My Recordings','url',131,xbmc.translatePath(os.path.join('special://home/addons/plugin.video.savvytvlite', 'art', 'f_record.jpg')), '', '')
    addDir('Favourite Channels','url',415,xbmc.translatePath(os.path.join('special://home/addons/plugin.video.savvytvlite', 'art', 'my_channels.jpg')), '', '')
    session_id = xbmcgui.Window(10000).getProperty("session_id")
    url = "%s%s%s" % (base_url,'/tv/api/groups?session_key=', (session_id))
    link = open_url(url)
...
def download_rec(name, url, iconimage):
    ...
    if directory == "notset" or directory == "":
        ...
        if os.path.exists(data_path):
            notification('Savvy TV Lite - Download started', name.upper(), '5000', iconart)
        else:
            notification('Savvy TV Lite - Download failed', name.upper(), '5000', iconart)
...
class DownloadThread(Thread):
    ...
    def run(self):
        ...
        notification('Savvy TV Lite - Download finished', name.upper(), '5000', iconart)
...
def on_demand_list(url):
    ...
    for a in all_videos:
        ...
        if ' Series' in title:
            ...
            addDirPlayable(title,url,203,thumb,"",plot, "", "od")
    addDir("[COLOR gold]>> Next Page[/COLOR]",np_url,201,xbmc.translatePath(os.path.join('special://home/addons/plugin.video.savvytvlite', 'art', 'next.png')), '','')
    setView('episodes', 'episodes-view')
...
def cartoons(name,url):
    ...
    addDir('Disney Junior Videos','http://www.disney.co.uk/disney-junior/content/video.jsp',301,xbmc.translatePath(os.path.join('special://home/addons/plugin.video.savvytvlite', 'art', 'disney_junior.jpg')), '', '')
    ...
    addDir('Disney Classic','http://gdata.youtube.com/feeds/api/users/UCa0h983kQj5OYa06gYhxgiw/uploads?start-index=1&max-results=50',395,xbmc.translatePath(os.path.join('special://home/addons/plugin.video.savvytvlite', 'art', 'mickey.gif')),'','')
...
def disney_jr(url):
    ...
    for c in categories:
        ...
        if name == 'A POEM IS HOME IN THE FASHION':
            thumb = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.savvytvlite', 'art', 'disney_junior.jpg'))
...
def disney_playlist(name, url, iconimage):
    dp = xbmcgui.DialogProgress()
    dp.create("Savvy TV Lite",'Creating Playlist')
    playlist = []
    link = open_url(url)
...
def create_strm_file(name, url, mode, dir_path, iconimage):
    ...
    if not os.path.exists(path):
        ...
        scan_library()
    #except:
        #xbmc.log("[Savvy TV Lite] Error while creating strm file for : " + name)
...
def addDirPlayable(name,url,mode,iconimage,ch_fanart, description, start, function):
        ...
        if function != 'od' and function != 'gb'and function != 'djr' and function != 'ng' and function != '' and function != 'favlist':
            ...
            contextMenuItems.append(("Add to Savvy TV Lite Favourites",'XBMC.RunPlugin(%s?name=%s&url=%s&mode=410&iconimage=%s&ch_fanart=%s)'%(sys.argv[0],urllib.quote(name),urllib.quote(url),urllib.quote(iconimage),ch_fanart)))
        if function == 'favlist':
            contextMenuItems.append(("Remove from Savvy TV Lite Favourites",'XBMC.RunPlugin(%s?name=%s&url=%s&mode=416&iconimage=%s&ch_fanart=%s)'%(sys.argv[0],urllib.quote(name),urllib.quote(url),urllib.quote(iconimage),ch_fanart)))
        if function == 'ng':
